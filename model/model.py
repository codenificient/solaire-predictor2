...
def predict_demand_pipeline(country_code, year):
    modelname = f"{BASE_DIR}/linear_gdp_growth-{__version__}.pkl"
    ...
    predicted_value = linear_model.predict(user_country_code_encoded)
    adjusted_predicted_value_rmse = predicted_value * 1.2
    print(f'Predicted Energy Usage for {user_country_name} in {year}: {round(adjusted_predicted_value_rmse[0], 2)}')
